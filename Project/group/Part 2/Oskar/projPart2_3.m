...
M = 1000; % number of OFDM symbols we want to transmit
...
for i = 1:length(E) % (loop with varying E values to plot SER over SNR)
    ...
    SNR(i) = 10*log10(E(i)*pathLoss/(N0));
    SERtheo(i) = 2*qfunc(sqrt(E(i)*pathLoss/(N0)));
...
%         SER(i) = ratio;
        ...
        SNR_subcarrier(:,i) = mean(abs(OFDMsymbols).^2,2)/N0; % average subcarrier SNR
end
avgSNR = mean(SNR_subcarrier,1);
%% Plot SER vs SNR
% check simulated SNR vs theoretical SNR
figure()
plot(E,SNR,'b'), hold on, grid on
plot(E,10*log10(avgSNR))
figure()
