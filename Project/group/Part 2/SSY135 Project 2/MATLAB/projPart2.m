...
OFDMsymbols = zeros(N,M,length(E));
for i = 1:length(E) % (loop with varying E values to plot SER over SNR)
    ...
    for m = 1:M
        ...
        OFDMsymbols(:,m,i) = s; % store the symbols in this matrix.
    end
    % Reciever part
    recievedOFDM = reshape(OFDMsymbols(:,:,i),[N*M 1]);
    ...
    SNR_subcarrier(:,i) = 10*log10(mean(abs(OFDMsymbols(:,:,i)).^2,2)/(N0)); % average subcarrier SNR in dB
end
avgSNR = mean(SNR_subcarrier,1); % average SNR in dB
%% Plot some scatterplots
if mode == 0
    scatterplot(OFDMsymbols(2,:,7)), grid on
    axis([-3.5e-6 3.5e-6 -3.5e-6 3.5e-6])
    title('Rx samples over subcarrier N = 2')
    saveas(gcf,'AWGNN2','epsc')
    scatterplot(OFDMsymbols(6,:,7)), grid on
    axis([-3.5e-6 3.5e-6 -3.5e-6 3.5e-6])
    title('Rx samples over subcarrier N = 6')
    saveas(gcf,'AWGNN6','epsc')
    scatterplot(OFDMsymbols(29,:,7)), grid on
    axis([-3.5e-6 3.5e-6 -3.5e-6 3.5e-6])
    title('Rx samples over subcarrier N = 29')
    saveas(gcf,'AWGNN29','epsc')
    scatterplot(OFDMsymbols(53,:,7)), grid on
    axis([-3.5e-6 3.5e-6 -3.5e-6 3.5e-6])
    title('Rx samples over subcarrier N = 53')
    saveas(gcf,'AWGNN53','epsc')
    scatterplot(OFDMsymbols(78,:,7)), grid on
    axis([-3.5e-6 3.5e-6 -3.5e-6 3.5e-6])
    title('Rx samples over subcarrier N = 78')
    saveas(gcf,'AWGNN78','epsc')
end
if mode == 1
    scatterplot(OFDMsymbols(2,:,7)), grid on
    axis([-3.5e-6 3.5e-6 -3.5e-6 3.5e-6])
    title('Rx samples over subcarrier N2 = 2')
...
%% plot SNR vs E
figure()
semilogx(E,avgSNR,'r--','LineWidth',2), grid on, hold on
semilogx(E,SNR,'k--','LineWidth',2)
title('SNR vs symbol energy E')
xlabel('log(E)'), ylabel('SNR [dB]')
legend('Average simulation SNR','Theoretical SNR','Location','NorthWest')
saveas(gcf,'SNRvsE','epsc')
