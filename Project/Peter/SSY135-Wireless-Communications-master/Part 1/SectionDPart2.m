...
Ns = 1e4;                       %number of samples per channel
N = 300;
M = 64;
fDTs =[0.1,0.005];
L = [1 2 3];
kc =[0 1 10];
figure(1), hold on % init figure
colormap('jet')
for k=1:3 % For all kc
    cla % clear data (done for plotting)
    for j=1:2 % For all fDTs
        fprintf('Loading: kc=%d, fDTs=%0.3f \n',kc(k),fDTs(j))

        %L = 1
        c_1(1,:) = channelBySpectrum(Ts,Ns,fDTs(j)/Ts) +kc(k)*ones(1,Ns); % channel by filter + kc
        c_1(2,:) = zeros(1,Ns); % Can not make it work without this vector in c_1
        ...
        c_2 = zeros(L(2),Ns);
        for l=1:L(2)
            temp = channelBySpectrum(Ts,Ns,fDTs(j)/Ts) +kc(k)*ones(1,Ns); % channel by filter + kc
            c_2(l,:) = temp;
        end
        ...
        sgtitle(['kc = ' + string(kc(k))+ ', fDTs = ' + string(fDTs(j))])
        %plot for L=1
        subplot(3,2,1)
        ...
        disp('Done, waiting for buttonpress')
        waitforbuttonpress % Wait for buttonpress to update figure
    end
end
